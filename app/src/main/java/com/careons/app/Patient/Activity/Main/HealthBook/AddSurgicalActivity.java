...
public class AddSurgicalActivity extends AppCompatActivity
        implements DatePickerDialog.OnDateSetListener, View.OnClickListener {

    private static SurgicalHistory surgicalHistory;
    ...
    private TextView problemIdTextView, problemNameTextView, problemHyperlinkTextView, problemCategoryTextView, editableTextViewDate, saveContentTextView;
    private EditText mSearchAddEditText, commentsEditText;
    private CheckBox isSufferingCheckbox;

    private Timer timer = new Timer();
    private final long DELAY = 1; // in ms
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hb_add_surgical_history);

        // To handle back button
        toolbar = (Toolbar)findViewById(R.id.toolbar);
        toolbar.setTitle(getString(R.string.add_surgical_history));
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            window.setStatusBarColor(getResources().getColor(R.color.darker_sh));
        }
        ...
        problemCategoryTextView = (TextView) findViewById(R.id.problem_category);
        ...
        switch (v.getId()) {
            ...
            case R.id.save_content:
                ...
                if (Validation.validateStartDate(editableTextViewDate, getApplicationContext())) {
                    ...
                    selectedProblemData.put(StringConstants.KEY_PROBLEM_CATEGORY, problemCategoryTextView.getText().toString());
                    ...
                    if (SurgicalHistory.find(SurgicalHistory.class, "patient_id = ? and problem_Id = ?",
                        ...
                        // Found
                        selectedProblemData.put(StringConstants.KEY_P_ID, surgicalHistory.getpId());
                        SurgicalFragment.updateSurgicalHistory(getApplicationContext(), selectedProblemData);

                    } else {

                        // Not Found
                        SurgicalFragment.createSurgicalHistory(getApplicationContext(), selectedProblemData);
                    }
    ...
    public void openDatePicker() {
        ...
        DatePickerDialog dpd = DatePickerDialog.newInstance(AddSurgicalActivity.this,
     ...
     */
    public void fetchSearchResults(final String searchQuery) {
        ...
        final String url = String.format(ServiceUrls.KEY_HEALTHBOOK_SEARCH,
                StringConstants.KEY_SURGICAL_PROCEDURES,
                    ...
                    try {
                        ...
                        if (response.has(StringConstants.KEY_VALUES)
                            ...
                            for (int i = 0; i < data.length(); i++) {
                                ...
                                searchResult.put(StringConstants.KEY_SEARCH_LIST_HYPERLINK, jsonObject.getString(StringConstants.KEY_SEARCH_LIST_HYPERLINK));
                                searchResult.put(StringConstants.KEY_CATEGORY, jsonObject.getString(StringConstants.KEY_SEARCH_LIST_CATEGORY));

                                initialSurgicalHistoryList.add(searchResult);

                                /**
                                 * Bind adapter to the result
                                 */
                                mAddSearchlistView.setAdapter(
                                        new SearchListAdapter(AddSurgicalActivity.this, initialSurgicalHistoryList,
                                                StaticConstants.SURGICAL_ADAPTER));
                            }
    ...
    public void searchListItemClicked() {
        mAddSearchlistView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                // Set selected problem name, Id and category
                problemNameTextView.setText(initialSurgicalHistoryList.get(position).get(StringConstants.KEY_NAME));
                problemIdTextView.setText(initialSurgicalHistoryList.get(position).get(StringConstants.KEY_ID));
                problemHyperlinkTextView.setText(initialSurgicalHistoryList.get(position).get(StringConstants.KEY_SEARCH_LIST_HYPERLINK));
                problemCategoryTextView.setText(initialSurgicalHistoryList.get(position).get(StringConstants.KEY_CATEGORY));
                 ...
                 */
                List<SurgicalHistory> surgicalHistoryList = SurgicalHistory.find(SurgicalHistory.class, "patient_id = ? and problem_id = ?",
                        SharedPreferenceService.getValue(getApplicationContext(), StringConstants.KEY_PATIENT_ID),
                        problemId);

                // See if record already exists to prepopulate data
                if (surgicalHistoryList.size() > 0) {

                    // Record Found
                    surgicalHistory = surgicalHistoryList.get(0);

                    editableTextViewDate.setText(surgicalHistory.getStartDate());
                    isSufferingCheckbox.setChecked(surgicalHistory.isStillSuffering());
                    commentsEditText.setText(surgicalHistory.getComments());
                }
            }
        });
    }
