...
public class AddGynaecologicalActivity extends AppCompatActivity
        implements DatePickerDialog.OnDateSetListener, View.OnClickListener {

    private static Gynaecology gynaecology;
    ...
    private TextView problemIdTextView, problemNameTextView, problemHyperlinkTextView, editableTextViewDate, saveContentTextView;
    private EditText commentsEditText, mSearchAddEditText;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hb_add_gynaecological);

        // To handle back button
        toolbar = (Toolbar) findViewById(R.id.toolbar);
        toolbar.setTitle(getString(R.string.add_gynae_history));
        ...
        switch (v.getId()) {
            ...
            case R.id.save_content:
                ...
                if (Validation.validateStartDate(editableTextViewDate, getApplicationContext())) {
                    ...
                    List<Gynaecology> gynaecologyList = Gynaecology.find(Gynaecology.class, "patient_id = ? and problem_id = ?",
                            SharedPreferenceService.getValue(getApplicationContext(), StringConstants.KEY_PATIENT_ID),
                            problemIdTextView.getText().toString());


                    // Check if record already exists
                    if (gynaecologyList.size() > 0) {

                        selectedProblemData.put(StringConstants.KEY_P_ID, gynaecology.getpId());

                        // Found -> Check Calling Activity
                        if (getIntent().getBooleanExtra(StringConstants.KEY_INONBOARDING, false)) {

                            OnboardingGynaecologicalIssuesActivity.updateGynaecologicalProblem(getApplicationContext(), selectedProblemData);
                            setResult(RESULT_OK, new Intent());

                        } else {

                            GynaecologicalFragment.updateGynaecologicalProblem(getApplicationContext(), selectedProblemData);
                            setResult(RESULT_OK, new Intent());
                        ...
                        if (getIntent().getBooleanExtra(StringConstants.KEY_INONBOARDING, false)) {
                            ...
                            OnboardingGynaecologicalIssuesActivity.createGynaecologicalProblem(getApplicationContext(), selectedProblemData);
                            setResult(RESULT_OK, new Intent());

                        } else {

                            GynaecologicalFragment.createGynaecologicalProblem(getApplicationContext(), selectedProblemData);
                            setResult(RESULT_OK, new Intent());
                        }
                    }
    ...
    public void openDatePicker() {
        ...
        DatePickerDialog dpd = DatePickerDialog.newInstance(AddGynaecologicalActivity.this,
                    ...
                    try {
                        ...
                        if (response.has(StringConstants.KEY_VALUES)
                            ...
                            for (int i = 0; i < data.length(); i++) {
                                ...
                                initialGynaecologicalList.add(searchResult);
                                ...
                                mAddSearchlistView.setAdapter(
                                        new SearchListAdapter(AddGynaecologicalActivity.this, initialGynaecologicalList,
    ...
    public void searchListItemClicked() {
        mAddSearchlistView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                problemNameTextView.setText(initialGynaecologicalList.get(position).get(StringConstants.KEY_NAME));
                problemIdTextView.setText(initialGynaecologicalList.get(position).get(StringConstants.KEY_ID));
                problemHyperlinkTextView.setText(initialGynaecologicalList.get(position).get(StringConstants.KEY_SEARCH_LIST_HYPERLINK));
                ...
                if (gynaecologyList.size() > 0) {
                    ...
                    // Record Found
                    gynaecology = gynaecologyList.get(0);

                    editableTextViewDate.setText(gynaecology.getStartDate());
                    isSufferingCheckbox.setChecked(gynaecology.isStillSuffering());
                    commentsEditText.setText(gynaecology.getComments());
                }
            }
        });
    }
