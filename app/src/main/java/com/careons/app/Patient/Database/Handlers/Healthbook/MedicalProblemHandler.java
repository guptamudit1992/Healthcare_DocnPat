...
public class MedicalProblemHandler
    ...
    // Constructor
    public MedicalProblemHandler(final MedicalProblem medicalProblem, final ViewDataBinding dataCard, final int sender) {

        this.medicalProblem = medicalProblem;
        this.dataCard = dataCard;
        MedicalProblemHandler.sender = sender;
        ...
        // Initialize Date fields (Image and Text)
        ...
        comments.setVisibility(Validation.isEmpty(medicalProblem.getComments()) ? View.GONE : View.VISIBLE);
                       ...
                       case R.id.action_delete:
                           ...
                           acceptCardView.setOnClickListener(new View.OnClickListener() {
                               ...
                               public void onClick(View v) {
                                   ...
                                   // Call Delete API
                                   if (sender == StaticConstants.MEDICAL_PROBLEM_ADAPTER) {
                                       MedicalProblemFragment.deleteMedicalProblem(context, String.valueOf(medicalProblem.getPId()));
                                   } else {
                                       OnboardingMedicalProblemsActivity.deleteMedicalProblem(context, String.valueOf(medicalProblem.getPId()));
                                   }
    ...
    public void saveDate(View view) {
        ...
        updatedDataObject.put(StringConstants.KEY_PROBLEMS_ID, medicalProblem.getProblemId());
        updatedDataObject.put(StringConstants.KEY_PROBLEM_NAME, medicalProblem.getProblemName());
        ...
        /**
         * Check if value present in local database (Problem ID)
         */
        List<MedicalProblem> medicalProblemDBList =
                MedicalProblem.find(MedicalProblem.class, "patient_id = ? and problem_id = ?",
                        SharedPreferenceService.getValue(context, StringConstants.KEY_PATIENT_ID),
                        String.valueOf((medicalProblem.getProblemId())));
         ...
         */
        if (sender == StaticConstants.ONBOARDING_MEDICAL_PROBLEM_ADAPTER) {

            if (medicalProblemDBList.size() > 0) {

                // Found -> Update Record
                updatedDataObject.put(StringConstants.KEY_P_ID, medicalProblem.getPId());
                OnboardingMedicalProblemsActivity.updateMedicalProblem(context, updatedDataObject);

            } else {

                // Not Found -> Create Record
                OnboardingMedicalProblemsActivity.createMedicalProblem(context, updatedDataObject);
            }

        } else {

            // From Healthbook -> Update Medical Problems List
            if (medicalProblemDBList.size() > 0) {

                // Found -> Update Record
                updatedDataObject.put(StringConstants.KEY_P_ID, medicalProblem.getPId());
                MedicalProblemFragment.updateMedicalProblem(context, updatedDataObject);

            } else {

                // Not Found -> Create Record
                MedicalProblemFragment.createMedicalProblem(context, updatedDataObject);
            }
        }
    }
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.update_status:
                // Manage Update Status of isSuffering Flag
    ...
    public void openDatePicker() {
        ...
        if (sender == StaticConstants.ONBOARDING_MEDICAL_PROBLEM_ADAPTER) {
            // From Onboarding
            dpd.show(OnboardingMedicalProblemsActivity.getInstance().getFragmentManager(), StringConstants.KEY_DATEPICKER_DIALOG);
        } else {
            // From Healthbook
            dpd.show(MedicalProblemFragment.getInstance().getActivity().getFragmentManager(), StringConstants.KEY_DATEPICKER_DIALOG);
        }
    ...
    public void showCardEditMenu() {
        ...
        if (sender == StaticConstants.MEDICAL_PROBLEM_ADAPTER || medicalProblem.isUpdate()) {
    ...
    public void bindSpinner() {
        ...
        int selectionPosition= adapter.getPosition(medicalProblem.getDuration());
